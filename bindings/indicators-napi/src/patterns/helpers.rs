...
/// Find Peaks
///
/// Finds peak indices — local maxima over `lookaround` bars.
#[napi]
pub fn find_peaks(values: Float64Array, lookaround: u32) -> Vec<u32> {
	find_peaks_internal(values.as_ref(), lookaround as usize)
		.iter()
		.map(|&x| x as u32)
		.collect()
}
...
/// Find Troughs
///
/// Finds trough indices — local minima over `lookaround` bars.
#[napi]
pub fn find_troughs(values: Float64Array, lookaround: u32) -> Vec<u32> {
	find_troughs_internal(values.as_ref(), lookaround as usize)
		.iter()
		.map(|&x| x as u32)
		.collect()
}
...
/// Zig Zag Filter
///
/// Zig Zag filter — filters out moves smaller than `deviation`.
#[napi]
pub fn zig_zag_filter(values: Float64Array, deviation: f64) -> Vec<f64> {
	zig_zag_filter_internal(values.as_ref(), deviation)
}
