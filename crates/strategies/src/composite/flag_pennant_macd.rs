...
pub fn flag_pennant_macd_strategy(
	...
	lows: &[f64],
	closes: &[f64],
	config: Option<FlagsPennantsMacdConfig>,
) -> StrategyResult<Vec<i8>> {
	let cfg = config.unwrap_or_default();

	let pole_length = cfg.pole_length.unwrap_or(10);
	let consolidation_bars = cfg.consolidation_bars.unwrap_or(10);
	let breakout_threshold = cfg.breakout_threshold.unwrap_or(0.02);
