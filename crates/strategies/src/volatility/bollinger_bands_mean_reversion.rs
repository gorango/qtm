...
pub fn bollinger_bands_mean_reversion_strategy(
	...
	if !(2..=100).contains(&period) {
		return Err("Bollinger Bands Mean Reversion period must be between 2 and 100".to_string());
	}
	if !(0.1..=5.0).contains(&std_dev) {
		return Err(
			"Bollinger Bands Mean Reversion std_dev must be between 0.1 and 5.0".to_string(),
		);
	}
